...
int player_read(player* this, int*i, int*j, int*d) {
	...
	do {
		...
		if (FD_ISSET(this->pout[0], &rdfs)) {
			...
			fprintf(stderr, "%ld, %ld\n", this->reloj.tv_sec, this->reloj.tv_usec);
...
void game_show(game* this) {
	int i, j, n = this->n;
	forn(i,n) { forn(j,n) fprintf(stderr, "%c", ".#X"[this->tbl[i][j]+1]); fprintf(stderr, "\n"); }
...
int game_step(game* this) {
	...
	fprintf(stderr, "JUDGE: Player %d - TL: %ld:%.2ld.%.3ld\n", this->cp, p->reloj.tv_sec/60, p->reloj.tv_sec%60, p->reloj.tv_usec/1000);
	if (this->lmd != -1) {
