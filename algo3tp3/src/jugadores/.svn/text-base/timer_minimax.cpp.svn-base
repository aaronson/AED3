...
int calcular_hacia_abajo (){
	...
	forn(i,jugposibles.size()){
		...
		plies++;
		//cerr << plies;
		int score = -calcular_hacia_abajo();
	
		if (score > maximo && plies == plies_original+1){
			jugada_actual = jug_a_probar;
		}
		maximo = max (score, maximo);
...
void terminar(int) {
	cerr << "Jugadas vistas totales: " << jugadas_vistas << endl;
	exit(0);
}
