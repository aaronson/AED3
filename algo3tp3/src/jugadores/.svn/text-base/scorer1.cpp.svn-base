...
bool puedo_poner_adyacente (int i, int j){
...
int puntuar_movida(){
	...
	forn(i,n){ 
		forn(j,n){
			if ((tbl[i][j] == c) && puedo_poner_adyacente(i,j)){
				scores[c] += 1;
			}else if ((tbl[i][j] == !c) && puedo_poner_adyacente(i,j)){
				scores[!c] += 1;
			}
		}
	}
...
void imprimir_jugadas(vjug jugposibles){
	//forn (i, jugposibles.size()){
		//cerr << jugposibles[i].jug.fila << " "  << jugposibles[i].jug.col << " "  << jugposibles[i].jug.orient << "  Valor: "  << jugposibles[i].puntaje << endl;
	//}
	//cerr << endl;
}
