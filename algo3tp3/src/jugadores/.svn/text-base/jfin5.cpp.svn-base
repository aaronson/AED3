...
void juega_movida(int k) {
	...
	//cerr << plies << endl;
	int prof_param;
	if (plies < primeras_jugadas){
		prof_param = 1;
	}else if (plies < (n*n)/2){
		prof_param = profund_media;
	}else prof_param = profund_final;
	if (plies == 0 || (plies == 1 && tbl[n/2][n/2] == -1 && tbl[n/2][(n/2)+1] == -1)){
		jugada_actual = (jugada){n/2, n/2, 0};
	}else{
	calcular_hacia_abajo(VALOR_MINIMO, VALOR_MAXIMO, prof_param);
	}
	entre = false;
...
int main(int argc, char* argv[]) {
	...
	primeras_jugadas = atoi(argv[1]);
	profund_media = atoi (argv[2]);
	profund_final = atoi (argv[3]);
	valorficha = atoi (argv[4]);
	valorady = atoi(argv[5]);
	valorexp = atoi(argv[6]);
	cin >> n >> c;
