...
int calcular_hacia_abajo (int alfa, int beta, int arbolrest, bool primiter){
	...
	if (primiter){
		//cerr << "ENTREEEEEEEEEEEEEEEEEEEEEEEEEEEE" << endl;
		...
		forn(i, jugposibles.size()){
			...
			plies++;
			int score = puntuar_movida();
			if (score >= maximo - offset) jugbuenas.push_back(jugposibles[i]);
			quita_movida(jug_a_probar.fila, jug_a_probar.col, jug_a_probar.orient);
			plies--;
			
		}
		jugposibles = jugbuenas;
	}
	//cerr << jugbuenas.size() << endl;
	...
	forn(i,jugposibles.size()){
		...
		plies++;
		//cerr << plies;
		int score = -calcular_hacia_abajo(-beta, -alfa, arbolrest-1, false);
...
void juega_movida(int k) {
	...
	calcular_hacia_abajo(VALOR_MINIMO, VALOR_MAXIMO, profund, true);
